...
进入到dev目录，打开文件subscribe2proxies.sh修改curl "自定义链接"
执行脚本sh subscribe2proxies.sh，会重新修改config.yaml文件；
复制dev目录下新的文件覆盖配置：cp dev/config.yaml config/config.yaml
然后运行`restart.sh` 脚本进行重启
